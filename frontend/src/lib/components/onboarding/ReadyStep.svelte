<script lang="ts">
    ...
    import { Sparkles, MessageCircle, LayoutGrid, ArrowLeft, Check } from 'lucide-svelte';
    ...
    async function completeOnboarding() {
        ...
        contextStore.setEmotionalContext(onboardingFlowStore.data.emotionalContext);
...
<div class="space-y-8" in:fade={{ duration: 400 }}>
    <div class="text-center space-y-4">
        <div class="mx-auto w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/30 mb-2">
            <Sparkles size={32} />
        </div>
    ...
    <div class="max-w-lg mx-auto space-y-4" in:fly={{ y: 20, duration: 400, delay: 100 }}>
        <!-- Summary of choices -->
        <div class="p-4 bg-white/5 rounded-2xl border border-white/10 space-y-3">
            <h3 class="text-sm font-medium text-slate-400 uppercase tracking-wider">Your setup</h3>
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    <span class="text-slate-400">Role</span>
                    <span class="text-white capitalize">{onboardingFlowStore.data.role.replace('_', ' ')}</span>
                </div>
                {#if onboardingFlowStore.data.emotionalContext}
                    <div class="flex justify-between">
                        <span class="text-slate-400">Situation</span>
                        <span class="text-white capitalize">{onboardingFlowStore.data.emotionalContext}</span>
                    </div>
                {/if}
                {#if onboardingFlowStore.data.deceasedName}
                    <div class="flex justify-between">
                        <span class="text-slate-400">Honoring</span>
                        <span class="text-white">{onboardingFlowStore.data.deceasedName}</span>
                    </div>
                {/if}
            </div>
        </div>
        ...
        <div class="grid gap-3">
            ...
            >
                <div class="flex items-start gap-4">
                    ...
                    <div class="space-y-1">
                        <h3 class="font-medium text-white">Explore on my own</h3>
                        <p class="text-sm text-slate-400">
                            Go to the dashboard and browse modules at your own pace.
                        </p>
                    </div>
                </div>
            </button>
