<script lang="ts">
    ...
    onMount(async () => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/auth/magic-link/verify?token=${encodeURIComponent(token)}`);
...
<div class="min-h-screen bg-[#0F1115] flex items-center justify-center p-6 font-sans">
    <div class="max-w-2xl w-full" in:fade={{ duration: 600 }}>
        ...
        <div class="bg-slate-900/50 border border-slate-800 rounded-3xl p-10 backdrop-blur-sm">
            {#if status === 'verifying'}
                <div class="text-center py-12 space-y-6" in:fly={{ y: 20, duration: 300 }}>
                    <Loader2 size={56} class="text-teal-400 animate-spin mx-auto" />
                    <div class="space-y-3">
                        <h3 class="text-white font-bold text-3xl">Verifying your login...</h3>
            ...
            {:else if status === 'success'}
                <div class="text-center py-12 space-y-6" in:fly={{ y: 20, duration: 300 }}>
                    ...
                    <div class="space-y-3">
                        <h3 class="text-white font-bold text-3xl">Login Successful!</h3>
                        <p class="text-slate-300 text-lg">
            ...
            {:else if status === 'error'}
                <div class="text-center py-12 space-y-6" in:fly={{ y: 20, duration: 300 }}>
                    <div class="w-20 h-20 bg-red-500/20 rounded-full flex items-center justify-center mx-auto border border-red-500/30">
                        <AlertCircle size={48} class="text-red-400" />
                    </div>
                    <div class="space-y-3">
                        <h3 class="text-white font-bold text-3xl">Verification Failed</h3>
                        <p class="text-slate-300 text-lg">
                            {error}
                        </p>
                    </div>
